...
|----|-------------|------------------|
| DISC-01 | System polls watchlist ATS endpoints on a schedule that keeps discovery-to-feed within 5–15 minutes. | Reuse the existing nine-minute claim cadence, add provider-aware staggering, and impose invocation/page/detail budgets so one large source cannot delay another. [VERIFIED: `.planning/REQUIREMENTS.md`; `supabase/migrations/0037_us_workday_dashboard_queue.sql`] |
...
The principal planning issue is lifecycle orchestration, not UI. Migration-controlled admission must create exact Experimental records automatically, but the current scheduled claim selects Active companies only and `pollConnector` rejects non-Active companies; the current observation function also allows zero-count evidence for most providers and activates only a fixed provider set. The plan therefore needs a bounded, server-only Experimental observation path and a forward migration that extends provider checks, requires positive eligible evidence for these sources, and keeps operational polling off until activation. [VERIFIED: `supabase/functions/poll-tick/index.ts`; `supabase/functions/_shared/connectors.ts`; `supabase/migrations/0037_us_workday_dashboard_queue.sql`]
...
### Idempotent Catalog-to-Experimental Reconciliation

```sql
-- Forward migration sketch: source_key and identity values are reviewed constants.
insert into public.companies (
  name, ats_type, source_key, activation_state, activation_successes
)
select
...
where catalog.company_name = 'Morgan Stanley'
  and catalog.disposition = 'experimental'
on conflict (source_key) do update
set name = excluded.name
where public.companies.activation_state <> 'disabled';
```

The final migration must follow actual table/constraint names, must not resurrect a deliberately Disabled record, and must update the catalog and operational identity atomically. [VERIFIED: CONTEXT D-17, D-21; existing migration patterns]
...
| Migration | All ten catalog rows receive expected disposition; exact candidate rows reconcile idempotently; Disabled stays Disabled; no duplicate company; rollback is not assumed. |
...
| Natural poll gate | Release evidence records scheduler invocation, complete positive scope, healthy state, and a forced incomplete observation that cannot close jobs. |
...
|-------------------------|---------------------------|--------|
| Manual `verify-board` URL admission for a user-added company | Forward migration reconciles a frozen catalog row into an exact Experimental identity | Meets D-17 without creating a generic admin URL surface. [VERIFIED: current flow + CONTEXT] |
...
|---|-------|---------|---------------|
| A1 | Initial bounds of 2 companies, 2 pages, 4 details, 100 pages, and a 120-second stop-scheduling deadline are suitable starting values. | Pattern 4 | Too low may miss the 5–15 minute target; too high may exceed provider/runtime limits. Planner must make them configurable and verify with fixtures/live timing. |

All portal classifications, current code gaps, official runtime limits, and repository constraints were verified in this session; only the initial numeric tuning values remain an implementation assumption.

## Open Questions

1. **Can Goldman Sachs prove a positive eligible provider category without fallback?**
   - What we know: list/detail JSON exposes `jobFunction` and `division`, but observed values can be empty or not contain an allowed exact term. [VERIFIED: live official portal probe]
   - What's unclear: whether the complete current population contains at least one U.S. role whose trustworthy provider field matches D-12/D-13 and remains stable.
   - Recommendation: make a positive category/detail fixture and live D-23 proof a pre-admission checkpoint; otherwise keep Goldman Unsupported.

2. **What exact operational bounds meet both provider politeness and the 5–15 minute target?**
   - What we know: the hosted runtime has documented memory/duration/CPU/idle limits, and current polling processes up to ten companies concurrently. [CITED: https://supabase.com/docs/guides/functions/limits] [VERIFIED: current poller]
   - What's unclear: production latency and eligible population sizes after category/U.S. filtering.
   - Recommendation: start conservatively, record per-slice timing/counts, and tune only from final-release evidence without weakening completeness.

3. **Should eligible provider category evidence be persisted per job or only in observation evidence?**
   - What we know: current `NormalizedJob` does not retain a general provider-category field, but activation/UAT must prove category mapping end to end. [VERIFIED: `adapters/types.ts`; CONTEXT D-23/D-25]
   - What's unclear: the minimum durable audit shape needed after normalization.
   - Recommendation: planner should include a small immutable provenance field or observation evidence table/digest that can map a normalized job ID to reviewed provider category and country evidence; do not rely only on logs.
