...
**Status:** PASS — migrations 0053–0055, hosted catalogs, exceptional cleanup,
and local gates are verified; the final behavior proof awaits exact approval.
...
## Behavior and cleanup authority awaiting approval

If approved with the exact signal below, the next continuation may:

1. Recompute every listed source, catalog, fixture, and scope hash and require
   target `fjcsvajkkztvlrpdplwx`.
2. Discover the service-role key non-interactively and retain it only in
   memory.
3. Seed exactly 2 disposable Auth users, 1 closed fake company/job lineage,
   2 user-job rows, and 2 resume rows.
4. Use two independent ordinary publishable-key sessions for all tracker
   behavior assertions, deriving exactly 4 application IDs and 5 event IDs
   into memory-only verified lineage.
5. Remove exactly 1 manifest-bound source row during snapshot-survival proof.
6. Run application-first, FK-safe cleanup; delete the 2 disposable Auth users
   last; require zero residue across the seven approved relations.

Any target, source byte, catalog digest, count, lineage, authority, cleanup, or
residue drift blocks behavior and Plans 04-04/04-05.
