...
describe('Paylocity verifier static safety contract', () => {
  ...
  it('uses exact ownership, immutable job, feed, one-call, and cron CAS gates', () => {
    ...
    expect(source).toContain('one current preference-pass Dashboard scope')
    expect(source).toContain("feed.includes('preferenceVisible')")
    expect(source).toContain("feed.includes('tierPresentation')")
    expect(source).not.toContain("feed.includes('defaultVisible')")
    expect(source).not.toContain('Focused predicate was not evaluated')
    expect(source).not.toContain('existing All-jobs predicate')
