...
describe('truthful company-name ingestion', () => {
  ...
  it('persists bounded Adzuna source company names on insert and exact refresh', () => {
    ...
    expect(source).toMatch(/source_company_name:\s*normalized\.companyName/i)
    const exactBranch = source.match(/if \(exactAction !== 'insert'[\s\S]*?continue\n\s*}/)?.[0]
    expect(exactBranch).toMatch(/source_company_name:\s*normalized\.companyName/i)
  })
  ...
  it('backfills joined names and retains tracked Greenhouse Ashby company identity', () => {
    ...
    expect(sql).toMatch(/update public\.jobs[\s\S]*source_company_name\s*=\s*(?:btrim\()?c\.name/i)
    expect(sql).toMatch(/from public\.companies c[\s\S]*j\.company_id\s*=\s*c\.id/i)
