...
describe('application route loading', () => {
  ...
  it('preserves every route and Dashboard scope behind one accessible fallback', () => {
    ...
    expect(mainSource).toContain('<RouteLoadBoundary>')
    expect(mainSource).toContain('role="alert"')
    expect(mainSource).toContain('This page couldn’t load')
    expect(mainSource).toContain('window.location.reload()')
    expect(mainSource).toContain('Reload page')
  })
})
